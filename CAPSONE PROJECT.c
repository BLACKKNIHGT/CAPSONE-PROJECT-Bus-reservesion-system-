...
void checkBusStatus(struct Bus buses[], int numBuses, int userId) {
    ...
    printf("\nSeat Status:\n");
    printf("    ");
    for (int i = 1; i <= buses[userId].totalSeats; ++i) {
        printf("%-3d", i);
        if (i % 10 == 0 && i != buses[userId].totalSeats)
            printf("| ");
    }
    printf("\n");

        if (buses[userId].seats[i] == 0) {0
            printf("[X]"); // Booked seat
        } else {
            printf("[ ]"); // Available seat
        }

        if ((i + 1) % 10 == 0 || i == buses[userId].totalSeats - 1)
            printf("\n");
    }
}
...
int main()
{
	...
	// Initialize bus data
	struct Bus buses[3] = {
		{ 101, "Nakuru", "Nairobi", 50, 50, 25.0 },
		{ 102, "Kisumu", "Mombasa", 40, 40, 20.0 },
		{ 103, "Eldoret", "Kiambu", 30, 30, 15.0 },
	};
	int numBuses = 3;
